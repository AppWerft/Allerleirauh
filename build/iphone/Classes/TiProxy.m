...
//Rare exceptions to indicate a bug in the _frontlineshop code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_frontlineshop = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_frontlineshop]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.frontlineshop.module"
